...
# Polynormer official two-phase schedule: local attention only for
# local_epochs (model._global stays False), then flip to global attention
# for global_epochs, warm-starting from the best-validation local checkpoint.
two_phase = args.model == 'polynormer' and (args.local_epochs + args.global_epochs) > 0
total_epochs = (args.local_epochs + args.global_epochs) if two_phase else args.epochs
if args.model == 'polynormer' and not two_phase:
    print("WARNING: Polynormer running WITHOUT the two-phase schedule: "
          "model._global stays False, so the global attention module is "
          "never trained or used (legacy behavior). Pass --local_epochs and "
          "--global_epochs to train the full published architecture.")
...
for run in range(args.runs):
    ...
    if args.model == 'polynormer':
        model._global = False
    if args.model == 'sgformer' and args.tr_weight_decay is not None:
        # official SGFormer: separate weight decay for the transformer branch
        # (params1) and the GNN branch + head (params2)
        optimizer = torch.optim.Adam(
            [{'params': model.params1, 'weight_decay': args.tr_weight_decay},
    ...
    for epoch in range(total_epochs):
        ...
        if two_phase and epoch == args.local_epochs:
            print("Switching Polynormer to global attention phase.")
            if args.save_model:
